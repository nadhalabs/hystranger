    ...
    >
      ...
      <div className="absolute left-3.5 top-3.5 z-10 flex items-center gap-1.5 rounded-full bg-black/60 px-3 py-1.5 text-xs font-semibold text-white backdrop-blur-md">
        ...
        <span className="flex items-center gap-0.5 opacity-80" aria-hidden="true">
          <span className={`h-2.5 w-0.5 rounded-full bg-white ${connected ? "animate-pulse" : ""}`} />
          <span className={`h-3.5 w-0.5 rounded-full bg-white ${connected ? "animate-pulse delay-75" : ""}`} />
          <span className={`h-2 w-0.5 rounded-full bg-white ${connected ? "animate-pulse delay-150" : ""}`} />
        </span>
      </div>
      ...
      {/* Top-Right Report Button (Desktop only if no PiP, or positioned cleanly) */}
      {!children && (
        <button
          type="button"
          onClick={onReport}
